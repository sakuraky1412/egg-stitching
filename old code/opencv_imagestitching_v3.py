...
def get_constrained_matches(ptsA, ptsB, width):
    ...
    for i, pt in enumerate(ptsA):
        if pt[1] > width * 3 / 4 and ptsB[i][1] < width * 1 / 4:
            if abs(pt[0] - ptsB[i][0]) < 50:
                matches.append((pt, ptsB[i]))
    return matches

def get_ROI(matches, height, thresholdA, thresholdB):
    matches_len = len(matches)
    ...
    if len(ROI) == 0:
        height_half = round(height * 1 / 2)
        ROI.append([(height_half, thresholdA), (height_half, thresholdB)])
    ROI = np.array(ROI)
    return ROI

def get_displacement(ROI):
...
def stitch_eggs(trainImg, queryImg, cImg, dImg, egg_name):
    ...
    ptsA = image_name_to_sift_features[egg_name + '_a_b'][0]
    ptsB = image_name_to_sift_features[egg_name + '_a_b'][1]
    ptsB_ = image_name_to_sift_features[egg_name + '_b_c'][0]
    ptsC = image_name_to_sift_features[egg_name + '_b_c'][1]
    ptsC_ = image_name_to_sift_features[egg_name + '_c_d'][0]
    ptsD = image_name_to_sift_features[egg_name + '_c_d'][1]
    ...
    matchesBC = get_constrained_matches(ptsB_, ptsC, width)
    matchesCD = get_constrained_matches(ptsC_, ptsD, width)
    # ROI = [matches[1], matches[2], matches[5]]
    ROI = get_ROI(matches, height, thresholdA, thresholdB)
    ROIBC = get_ROI(matchesBC, height, thresholdA, thresholdB)
    ROICD = get_ROI(matchesCD, height, thresholdA, thresholdB)
    # Get x and y displacements of the matched points for the two images
    displacementX, displacementY = get_displacement(ROI)
    displacementXBC, displacementYBC = get_displacement(ROIBC)
    displacementXCD, displacementYCD = get_displacement(ROICD)
    displacementX = -displacementX
    displacementY = abs(displacementY)
    displacementXBC = -displacementXBC
    displacementYBC = abs(displacementYBC)
    displacementXCD = -displacementXCD
    displacementYCD = abs(displacementYCD)
    ...
    for i in range(0, height):
        for j in range(thresholdA, width):
            if (movedQuery[i, j] > [5, 5, 5]).all():
                leftBorder[i][j] = True
    for i in range(0, height):
        for j in range(thresholdA, width):
            if (result[i, j] > [5, 5, 5]).all():
                rightBorder[i][j] = True
    # print('here')
    for i in range(0, height):
        for j in range(thresholdA, thresholdA + width):
            if (movedQuery[i, j] > [5, 5, 5]).all():
                if (result[i, j] < [15, 15, 15]).all():
                    result[i, j] = movedQuery[i, j]
                else:
                    if leftBorder[i, j] and j < width - (thresholdB / 2):
    ...
    leftWidth = suture - round(window / 2)
    rightWidth = suture + round(window / 2)
    for i in range(height * 1 / 4, height * 3 / 4):
        for j in range(leftWidth, rightWidth):
            if (resultCopy[i, j] > [10, 10, 10]).all() and (movedQuery[i, j] > [10, 10, 10]).all():
                a = (rightWidth - j) / window
                result[i, j] = resultCopy[i, j] * a + movedQuery[i, j] * (1 - a)
    ...
    cv2.imwrite(f"images/output/{egg_name}_stitched_debug.jpg", result)
...
# TODO: fix bug where edge of egg is dissappearing
