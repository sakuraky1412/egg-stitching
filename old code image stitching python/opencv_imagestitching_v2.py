...
def read_single_set_of_images(egg_name):
    ...
    dImg_gray = cv2.cvtColor(dImg, cv2.COLOR_BGR2GRAY)
    ...
    with open('matched_features.csv') as csv_file:
        ...
        for i, row in enumerate(csv_reader):
            ...
            if new_pair:
                ref_len = len(ref_id)
                if prev_egg_name == '':
                    prev_egg_name = ref_id[0:ref_len - 9]
                else:
...
def stitch_eggs(trainImg, queryImg, cImg, dImg, egg_name):
    ...
    cv2.imwrite(f"images/output/{egg_name}_stitched_debug2.jpg", result)
